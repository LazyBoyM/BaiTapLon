...
export default function CreateProduct() {
  ...
  // Hàm helper lấy config hiển thị tùy theo Category
  const getCategoryConfig = (category) => {
    switch (category) {
      case 'Phần mềm':
      case 'Template':
        return {
          demoLabel: 'Link Live Preview (Website demo)',
          demoPlaceholder: 'https://preview.your-theme.com',
          fileType: 'file',
          showGallery: true
        }
      case 'Nhạc':
        return {
          demoLabel: 'Link nghe thử (SoundCloud/Spotify/MP3)',
          demoPlaceholder: 'https://soundcloud.com/you/track',
          fileType: 'audio',
          showGallery: false
        }
      case 'Video':
        return {
          demoLabel: 'Link Video Trailer (YouTube/Vimeo)',
          demoPlaceholder: 'https://youtube.com/watch?v=...',
          fileType: 'video',
          showGallery: false
        }
      case 'Ảnh':
        return {
          demoLabel: 'Link xem trước thu nhỏ (Watermarked)',
      ...
      case 'Ebook':
        return {
          demoLabel: 'Link đọc thử (PDF Sample)',
          demoPlaceholder: 'https://drive.google.com/file/...',
          fileType: 'file',
          showGallery: false
        }
      default:
        return {
          demoLabel: 'Link xem trước / Preview (Không bắt buộc)',
          demoPlaceholder: 'https://...',
          fileType: 'other',
          showGallery: true
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Đăng sản phẩm thành công! Đang chờ admin duyệt.')
      navigate('/my-products')
    } catch (error) {
      toast.error(error.response?.data?.message || 'Không thể đăng sản phẩm')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-semibold text-gray-900">Đăng sản phẩm mới</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Phần 2: Tài nguyên (Động theo Category) */}
        {formData.category && (
          <Card padding="lg">
            <h2 className="text-lg font-medium text-gray-900 mb-4 flex items-center gap-2">
        ...
        <div className="flex gap-4 pt-4">
          <Button type="submit" loading={loading} className="flex-1 text-lg" size="lg" disabled={!formData.category}>
            {formData.category ? 'Đăng sản phẩm' : 'Vui lòng chọn Danh mục'}
          </Button>
